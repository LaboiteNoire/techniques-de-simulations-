...
\phantom{aaaaaaa} — \textit{On note} $\mu_0$, \textit{une mesure de probabilité sur $(E, \mathcal{A})$, dite « loi initiale de $(H_k)_{k \in \mathbb{N}^{*}}$ », telle que $(\mu_0(i))_{i \in [|1,3|]}$, est une permutation quelconque de $(1,0,0)$}.

\phantom{aaaaaaa} — \textit{On note} $Q \in M_3(\mathbb{R})$, \textit{une matrice stochastique, dite « matrice de transition de $(H_k)_{k \in \mathbb{N}^{*}}$ », telle que $\forall k \in \mathbb{N}^{*}$\footnotemark[1]} : 
...
« La \textit{méthode optimale} serait-elle fonction des conditions initiales? ». La question ne manque pas pertinence, après tout notre loi à support fini ne possède que 3 états. En ce sens une \textit{« Table de Walker »} est-elle réellement une méthode plus optimale que notre \textit{approche naïve}?
...
\phantom{aaaaaaa}
 ...
 — Pour $k \in \mathbb{N}$, la simulation d'une trajectoire a $k$ étapes, d'une \textit{chaine de Markov} à $n$ états serait de l'ordre de $o(knln(n))$ avec notre approche naïve et de $o(k)$ avec notre \textit{table de walker\footnotemark[3]}.
 
 — Simuler convenablement des conditions météorologiques à l'aide d'une unique \textit{Chaine de Markov} à 3 états, nous apparait fort improbable. Il est fort à parier que l'utilisateur sera amené à faire varier le nombre d'états : $n$. En ce sens, notre \textit{« modèle à Table de Walker »}, se montre beaucoup plus robuste que le précédent.
...
Notre modèle A repose sur une autre hypothèse : que l'état de la météo au jour $n$, dépend uniquement de l'état de la météo au jour $n-1$.

Nous n'expliquerons pas a quel point cette hypothèse est absurde, néanmoins la valeur d'un modèle ne se limite pas a la somme de la valeur de ses axiomes (ex : les modèles de base de la micro-économie).
\end{center}
...
\phantom{aaaaaaa} — \textit{On note $u$, l'investissement initial et individuel par assuré.}
...
\phantom{aaaa}— Deuxièmement car le code qui y est associé est lui même relativement succinct.
...
A terme, nous avons finalement opter pour une méthode de rejet (bien que la \textit{« table de walker »} nous ait un instant effleuré l'esprit).
...
La chaine $(H_k)_{k \in \mathbb{N}^{*}}$ étant \textit{irréductible} et son ensemble d'états $E$ fini, nous sommes assurés de l'existence et de l'unicité d'une \textit{« loi invariante »}, propre a $(H_k)_{k \in \mathbb{N}^{*}}$. 
...
\end{minipage}
\begin{minipage}[r]{0.8\textwidth}
Cette partie bénéficiera grandement des formules exposées en \hyperlink{ref2}{\textcolor{blue}{\textbf{\textit{$3^e$ partie}}}}, sur notre capacité à procéder étapes par étapes, afin d'optimiser notre code.

En premier lieu, nous générons nos possibles dates d'occurrences par une \textit{simili} méthode de rejet, puis nous générons la trajectoire de la \textit{Chaine de Markov} portant en elle l'information nécessaire a la bonne formation de nos critères de rejet.
...
\begin{center}
\includegraphics[width=12cm,height=3.4cm]{img20}
...
Par égard pour notre exercice premier, nous avons trouvé pertinente l'idée d'étudier le comportement de notre probabilité de ruine comparativement à nos niveaux de réserves($u$ et $c$). 

Ce résultat (conforme aux prédictions, il faut bien l'admettre), renforce d'autant plus la crédibilité de notre modèle.

\phantom{aaaa}

En effet, il parait assez intuitif que réduire nos réserves (que cela se fasse par le biais de de l'investissement initial $u$, ou par le biais du taux de prime journalière $c$), expose un peu plus l'assureur au risque d'insolvabilité (entendez par là que la probabilité de ruine serait une fonction décroissante de nos réserves). 

\phantom{aaaa}

Cela nous offre d'une certaine façon des perspectives nouvelles.
Notre modèle stochastique pouvant se révéler lourd par certains aspects, nous pourrions interpoler ses résultats, ou entrainer un modèle d'apprentissage statistique quelconque a prédire les sorties de ce dernier. 
\end{minipage}
\end{center}
...
\begin{center}
\includegraphics[width=12cm,height=3.4cm]{img21}
...
Néanmoins nous tenons par soucis d'honnêteté intellectuelle a signaler que les performances de l'une ou de l'autre paraissent avant tout contextuelles.
\end{minipage}
\end{center}
...
\begin{center}
\begin{minipage}[r]{0.6\textwidth}
lorsque $\delta$ est très grand, le \textit{terme oscillant} devient quasiment constant et la densité tend vers une loi normale tronquée. Dans ce cas, la méthode optimisée (\textit{rejet gaussien})
...
\begin{center}
\includegraphics[width=5cm,height=4.4cm]{img18}
\captionof{figure}{$\delta = 10000$}
\end{center}
\end{minipage}
\end{center}
...
\begin{minipage}[r]{0.42\textwidth}
\begin{center}
\includegraphics[width=5cm,height=4.4cm]{img19}
\captionof{figure}{$\delta = 1$}
\end{center}

\end{minipage}
\begin{minipage}[r]{0.56\textwidth}
A l'inverse, lorsque $\delta$ est très petit (entendez par la proche de $1$), les oscillations de la densité deviennent très marquées, ce
qui rend la méthode de rejet uniforme nettement moins efficace. Ces situations extrêmes
sont précisément celles où la comparaison entre les deux méthodes est la plus instructive.
\end{minipage}
\end{center}
...
Valeur de $\delta$ & Modèle uniforme & Modèle gaussien \\ \hline

$1$ & $1.17s$ & $0.35s$ \\ \hline

$10000$ & $0.88s$ & $0.28s$ \\ \hline


\end{tabular}
\end{center}

Cela vient confirmer que dans les cas
extrêmes testés notre deuxième méthode de rejet est bien plus performante.
\end{minipage}
\end{center}
...
\end{minipage}
\begin{minipage}[r]{0.8\textwidth}
Considérant nos implémentations relativement proches de celles du Modèle A, nous ne trouvions pas particulièrement pertinent de disserter longuement sur nos méthodes de simulation.

Nous insistons toutefois sur une limite inhérente a notre implémentation (ou peu s'en faut) du Modèle B dans sa seconde version. Les ramifications du modèle, trop nombreuses, amoindrissent nos capacités de parallélisation, les sous problèmes étant si nombreux que les périodes de préparation aux dites parallélisations font exploser le temps d'exécution de ce dernier.

\phantom{aaaa}

A cela vient s'additionner une limite fort incapacitante : l'impossibilité de pouvoir réinitialiser un \textit{thread python}.

Nous ne doutons pas qu'il puisse exister une solution satisfaisante à ce problème, nous ne l'avons simplement pas trouver par manque de temps.  
\end{minipage}
\end{center}
...
\textcolor{teal}{def} \textcolor{blue}{modele\_B\_meteo\_indep}(N\_assures, u, c, horizon, M, H0, lambd, p, x0, b, sigma, delta, a, alpha):
...
\phantom{aaaaaaaa}H\_i \textcolor{mymauve}{=} simuler\_chaine\_markov(M, H0, horizon)

\phantom{aaaaaaaa}sinistres\_i \textcolor{mymauve}{=} simuler\_sinistres\_avec\_temps(H\_i, lambd, p, x0, b, sigma, delta, a, alpha)
...
\phantom{aaaaaaaa}\textcolor{codegreen}{if} \textcolor{codegreen}{len}(T\_all) \textcolor{mymauve}{==} \textcolor{codegreen}{0}:

\phantom{aaaaaaaaaaaa}\textcolor{teal}{return} np.\textcolor{blue}{array}([]), np.\textcolor{blue}{array}([]), \textcolor{codegreen}{None}

\phantom{aaaaaaaa}T\_global \textcolor{mymauve}{=} np.\textcolor{blue}{concatenate}(T\_all)

\phantom{aaaaaaaa}X\_global \textcolor{mymauve}{=} np.\textcolor{blue}{concatenate}(X\_all)

\phantom{aaaaaaaa}indices\_tri \textcolor{mymauve}{=} np.\textcolor{blue}{argsort}(T\_global)

\phantom{aaaaaaaa}T\_global \textcolor{mymauve}{=} T\_global[indices\_tri]

\phantom{aaaaaaaa}X\_global \textcolor{mymauve}{=} X\_global[indices\_tri]

\phantom{aaaaaaaa}Rt \textcolor{mymauve}{=} N\_assures \textcolor{mymauve}{*} u \textcolor{mymauve}{+} N\_assures \textcolor{mymauve}{*} c \textcolor{mymauve}{*} T\_global \textcolor{mymauve}{-} np.\textcolor{blue}{cumsum}(X\_global)

\phantom{aaaaaaaa}indices\_ruine \textcolor{mymauve}{=} np.\textcolor{blue}{where}(Rt \textcolor{mymauve}{<} \textcolor{codegreen}{0})[\textcolor{codegreen}{0}]

\phantom{aaaaaaaa}t\_ruine \textcolor{mymauve}{=} \textcolor{codegreen}{None}

\phantom{aaaaaaaa}\textcolor{codegreen}{if} \textcolor{codegreen}{len}(indices\_ruine) \textcolor{mymauve}{>} \textcolor{codegreen}{0}:

\phantom{aaaaaaaaaaaa}t\_ruine \textcolor{mymauve}{=} T\_global[indices\_ruine[\textcolor{codegreen}{0}]]

\phantom{aaaaaaaa}\textcolor{teal}{return} T\_global, Rt, t\_ruine
...
\textcolor{teal}{def} \textcolor{blue}{run}(self, rounds\textcolor{mymauve}{=}\textcolor{codegreen}{365}):
...
\phantom{aaaaaaaaaaaaaaaa}self.\textcolor{blue}{T.append}(va\_transitoire)
...
\phantom{aaaa}self.\textcolor{blue}{reinit}()
\end{minipage}
\end{center}
...
La \textbf{\textit{fonction}} \textcolor{blue}{run} remplie la liste globale du portefeuille de sinistres individuels.

\phantom{aa}

La \textbf{\textit{fonction}} \textcolor{blue}{T\_run} génère la liste des occurrences.
...
\textcolor{teal}{def} \textcolor{blue}{T\_run}(self, draw\textcolor{mymauve}{=}\textcolor{orange!80}{True}):

\phantom{aaaa}\textcolor{codegreen}{for} i \textcolor{codegreen}{in range}(self.\textcolor{blue}{settings.parameters}[\textcolor{red}{"N"}]):

\phantom{aaaaaaaa}self.\textcolor{blue}{subsub.run}()

\phantom{aaaa}self.\textcolor{blue}{T.sort}()
...
\begin{center}
\includegraphics[width=12cm,height=3.4cm]{img22}
\captionof{figure}{Évolution de la probabilité de ruine en fonction de $u$ et de $c$ (Modèle B)}
\end{center}
...
Entre autres des \textit{Tables de Walker}, des méthodes de rejets, ou encore des méthodes d'\textit{inversions de fonctions de répartition}.
...
Pour ce faire nous proposons plusieurs solutions :
 ...
 — \textit{Interfaçer} notre code avec du code C++.
 
 — Massivement paralléliser nos calculs sur un GPU.

 — Entrainer un modèle d'apprentissage statistique quelconque à prédire les sorties de notre modèle.
...
Par ailleurs nos deux modèle reposent sur deux hypothèses encore plus discutables : l'une est que la sinistralité est homogène (la loi de $X$ est commune à tous les assurés et ne dépends pas de la météo), et l'autre que les montants sont indépendants d'un individus à un autre.

Or (que l'on parle de risques climatiques et ou météorologiques), l'impacte financier et économique n'est jamais le résultat d'une expérience purement individuelle.
...
\begin{center}
\href{https://github.com/priscilla1269}{\includegraphics[width=1cm,height=1cm]{img2}}
\end{center}
\end{minipage}
\begin{minipage}[r]{0.4\textwidth}
\begin{center}
\href{https://github.com/LaboiteNoire}{\includegraphics[width=1cm,height=1cm]{img2}}
\end{center}
